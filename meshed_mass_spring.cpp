...
int main(int argc, char** argv) {
  ...
  Color_listener* col2 = new Color_listener(&color4, &color5, &color6);
  Force_listener* wind = new Force_listener(&a, &b, &c);
  Move_listener<MeshType>* mov = new Move_listener<MeshType>(&mesh, &mesh2);
  ...
  viewer.add_listener(col2);
  viewer.add_listener(wind);
  viewer.add_listener(mov);

  
  for (double t = t_start; t < t_end; t += dt) {
         
    //Initialize forces
    WindForce wind_force(Point(a,b,c));
    WindForce wind_force2(Point(0,0,0));
    ...
    auto force2 = make_combined_force(MassSpringForce(), make_combined_force(pressure_force, damp_force, wind_force2));
    ...
    symp_euler_step(mesh2, t, dt, force2);
    ...
    viewer.add_edges(mesh.edge_begin(), mesh.edge_end(), node_map);
    viewer.add_nodes(mesh2.node_begin(), mesh2.node_end(), color(color4, color5, color6), node_map);
    viewer.add_edges(mesh2.edge_begin(), mesh2.edge_end(), node_map);
